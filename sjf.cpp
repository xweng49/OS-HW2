...
int processturn(processes[], int, int);
...
int main(int argc, char *argv[]) {
  ...
  do
  {
    turn=processturn(myprocess, turn, process);
    if (turn != -1)
    {
...
int processturn (processes proc[], int turn, int process)
{
  int lowctime = 1000;
  int pturn = -1;
  ...
  for (int i=0; i<process; i++)
  {
    if(proc[i].getcomptime() < lowctime && !proc[i].getfinished() && i!=turn)
    {
    lowctime = proc[i].getcomptime();
    pturn = i;
    }
  }
